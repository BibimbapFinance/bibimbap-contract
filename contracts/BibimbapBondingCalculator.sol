...
contract BibimbapBondingCalculator is IBibimbapBondingCalculator {
    ...
    function markdown( address _pair ) external view override returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == BBB ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20( BBB ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}
